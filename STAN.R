...
sigma_high ~ cauchy(0, 1);
sigma_low ~ cauchy(0, 1);
...
real <lower = 0> alpha;  // intercept for guest scoring
real  delta0;  // HFA for sport
...
real dev1[Nh]; // devation from average between leagues guest scoring
real delta1[Nh]; // HFA league

real dev2[Nl]; // devation from average by team guest scoring
real delta2[Nl]; // HFA team
...
stanDso = stan_model(model_code = mPo2) 
#===============  Data Feed ======================
...
fit = sampling(object = stanDso, data=dataList, 
               init=0.1, control=list(adapt_delta = 0.9),
               chains=4, iter=999, warmup=333, thin=1)
